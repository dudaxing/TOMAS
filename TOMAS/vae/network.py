...
class Encoder(nn.Module):
  ...
  def forward(self, x: torch.Tensor)-> torch.Tensor:
    ...
    if self.is_training:
      # randn_like is equivalent to sampling self.normal_dist but stays on
      # mu's device (lets the VAE train on GPU). Original used a CPU normal_dist.
      self.z = mu + sigma*torch.randn_like(mu)
    else:
      self.z = mu
