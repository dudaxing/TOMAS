...
**有效改进（已采用）：**
- **Pareto 暖启动连续化**（`scripts/run_pareto.py`）：按接触面积升序扫描，每个点用上一点收敛的网络权重热启动（同伦/连续化），消除独立优化的非单调噪声。注意：暖启动仅使前沿"单调但偏平"（28→31），**真正复现论文上升趋势的是体积约束**，见 §6.6。
...
| §3.5 Pareto 上升趋势 | 单调增 22→39 | **单调上升 20→25**（双约束，§6.6）| 趋势复现，低端贴合 |
...
## 6.6 Pareto 前沿（Fig 14）：平前沿的根因与修正

> 起因：对照论文 Fig 14，发现本复现扩散器 Pareto 前沿被"压平"，而非论文那样随接触面积单调上升。

**现象**（真实-FEA 功率）：暖启动·单约束 28.3/28.6/28.5/31.1（几乎水平）；独立·单约束 36.5/31.5/20.9/23.4（无趋势甚至反向，各点落入不同局部最优）；论文 22/25/32.5/39（单调上升）。

**根因（设计图证实）**：仅约束接触面积时，给定接触面积下的功率**欠定**——优化器把高周长微结构堆在**上下无流速壁面**（ca=80 设计图：星形只在壁排、中心全空），不挡主流道 → 接触面积增大而功率几乎不变。暖启动又把各点焊在最通透解附近，进一步压平。

**修正**：在 `run_to.py` 增 `--with-volume`，**同时**约束**实心体积分数**（`combined_loss` 原生支持多约束）。固定实心≥25%（对应 §3.1 的 vf≈0.25 微结构）后，微结构被迫**均布**（设计图填满全域），接触面积与渗透率重新耦合——孔越小渗透率越低、功率越高：
...
**结论**：双约束前沿**单调上升**，低端量级与论文高度吻合（20.0/22.2 vs 22/25）；高端上升较缓（25.3 vs 39），因本 VAE 的高周长微结构仍允许部分绕流，属微结构库固有局限。功率绝对量级可由实心率调节（实心≥50%→75–97；≥75%→152–216；≥25% 最贴论文）。

## 6.7 微结构词汇表修复：相关采样恢复各向异性叶片（第四轮）

> 起因：论文设计（如 Fig 16b）含大量**杏仁/叶片状**各向异性微结构，本复现却从未出现。**本轮重新生成数据并重训 VAE，前几节（§6.1–§6.6）的具体数值据此改变，以本节为最新结论；前几节保留为复现演进记录。**

**根因**：`generate_random_super_shapes` 对 n1,n2,n3 **独立均匀**采样于 [0.5,10]。叶片需 m≈2 且 n1=n2=n3 全部 <1，独立采样下 P(三者全<1)≈0.05³≈0.01% → **7000 样本中叶片 0 个**，数据被高 n 凸团（圆/星）主导，VAE 永远学不到叶片。

**修正（方案1）**：改为 `n1=n2=n3` **相关采样**（单次抽样），贴合论文图示（n1=n2=n3=0.6）。重生成后：低 n 样本 372、标准叶片 59、**强各向异性微结构 495 个**（修复前全 0）。

**效果**（重训 VAE）：
- **叶片/杏仁状回归**：扩散器设计右侧出现多个椭长/透镜/菱形各向异性形状；M\* 由各向同性圆变为 **m≈2、a≠b 的椭长各向异性微结构**（真实 C11≈4×C00）——论文的定向导流砖块。
- **意外大收获——VAE 自洽性质变**：分叉管解码 vs 真实-FEA 误差降到 **功率 1.33% / 接触面积 1.37%**（旧 VAE 在 M\* 处曾达 24×/82%）。因相关采样把属性空间从 3 维降到 1 维，2 维隐空间能忠实表示——从根上修复"解码器不自洽"弱点。
- **分叉管**（+体积约束）：填满全域的分叉结构，真实功率 38.7（验证误差 1.3%）。

**权衡（如实记录）**：弯管 §3.3b 功率由 6.6 升至 13.1（相关采样牺牲了旧库少数极高渗透形状，整体渗透率偏低）；Pareto 真实功率 42/32/34/37（接触面积 50/60/70/80）首点突跳、非单调（需多种子 best-of 打磨）。

**结论**：方案1 让微结构词汇表与论文对齐（各向异性叶片回归）并**显著改善 VAE 自洽性**（解码≈真实），是比前几轮调参更本质的修复；代价是个别绝对功率上升与 Pareto 需进一步打磨。

## 6.8 叶片主导库：收紧 m≈2 + 降 max_n（第五轮，最终库）

> 目标：让设计**尽可能像论文图片**（Fig 11/16 是叶片主导）。第四轮（§6.7）相关采样让叶片占 ~5%，设计仍非叶片主导。本轮把参数集中到 2 折叶片族。**本节为最终库，前几节具体数值据此再次更新。**

**配置**（`datagen.yaml`）：m∈[1.8,2.6]、n1=n2=n3∈[0.5,3.5]（相关）。渲染验证：100% 2 折**叶片/杏仁/椭圆族**（低 n 尖叶、中 n 杏仁、高 n 圆角卵），无多角星。数据集叶片(n<1) **1129/7000（16%）**。

**效果（重训 VAE，对标论文形态）**：
- **M\***：m≈2.18、a≠b、真实 C11≈**6.8×**C00——强定向叶片，对标 Fig 10。
- **§3.2 弯管仅取向**：全域同一叶片、仅取向变化引导绕流——**对标 Fig 11d**；功率 解码 14.4 / 真实 13.6 vs **论文 15.1（吻合）**。
- **§3.7 分叉管**（+体积）：中区定向叶片/透镜成流道 + 边缘卵形——**对标 Fig 16b**；验证误差 1.5%。
- **§3.4 扩散器**：叶片/透镜主导、沿流向定向排列。VAE 自洽性优秀（误差 1.25–1.5%）。

**权衡（如实记录）**：叶片/杏仁**周长比多角星小** → 扩散器接触面积**封顶约 45** → 论文 Fig 14 的 50–80 在叶片库下不可达。即"叶片主导外观"与"高接触面积可达"存在张力；**Fig 14 上升前沿仍以第三轮宽库（§6.6）为准**，本轮以叶片主导外观（Fig 10/11/16）为主。

**结论**：第五轮使 M\*/§3.2/§3.7/§3.4 设计**叶片主导、形态贴合论文**，§3.2 功率亦吻合（14.4 vs 15.1）；代价是叶片库接触面积上限较低，Fig 14 高端沿用宽库。
